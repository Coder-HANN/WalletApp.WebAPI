...
public class WithdrawCommandHandler : IRequestHandler<WithdrawRequestDTO, ServiceResponse<TransactionResponseDTO>>
{
    ...
    public async Task<ServiceResponse<TransactionResponseDTO>> Handle(WithdrawRequestDTO request, CancellationToken cancellationToken)
    {
        ...
        var wallet = await _walletRepository.GetAsync(w => w.Id == request.WalletId && w.AppUserId.ToString() == userId);
        if (wallet == null)
        ...
        var bankAccount = await _bankAccountRepository.GetAsync(b => b.Id == request.AppBankAccountId && b.AppUserId.ToString() == userId);
        if (bankAccount == null)
